...
$query = "SELECT a.no,a.semester,a.tahun,a.jlmh_sks,a.ip,a.ipk,b.id,b.nama,c.foto FROM kuliah a, user b, foto c where a.id=b.id and b.id=c.id";
    ...
    <table class="table border">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= '<img class="img" width ="100px" src="data:image/jpeg;base64,'.base64_encode( $data['foto'] ).'"/>' ?></td>
				<td><?= $data['nama'] ?></td>
                <td><?= $data['tahun'] ?></td>
                <td><?= $data['semester'] ?></td>
                <td align="center"><?= $data['jlmh_sks'] ?></td>
				<td><?= $data['ip'] ?></td>
				<td><?= $data['ipk'] ?></td>
                <td>
					<a href="<?= $WEB_CONFIG['base_url'] ?>tambah_kuliah-<?= $data['id']?>" class="btn btn-primary mb-3">
						...
						</svg> Tambah Data Kuliah
					</a>
                    <!-- PROSES EDIT MENGIRIMKAN ID DENGAN METHOD GET -->
                    <a href="<?= $WEB_CONFIG['base_url'] ?>update_kuliah-<?= $data['id'] ?>-<?= $data['no'] ?>" class="btn btn-success m-1">
                    ...
                    <a href="javascript:alertDelete(<?= $data['no'] ?>);" class="btn btn-danger m1">
                    <svg style="width:20px;height:20px" viewBox="0 0 24 24" class="mb-1">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a type="button" class="btn btn-danger" id="deleteButtonModal">hapus data!</a>
      </div>
    </div>
  </div>
</div>
...
function alertDelete(idn) {
    ...
    $('#deleteButtonModal').attr('href', '<?= $WEB_CONFIG['base_url'] ?>index.php?page=delete_kuliah&id='+idn);
